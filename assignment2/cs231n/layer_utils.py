...
def affine_norm_relu_forward(x,w,b, gamma, beta, bn_param, normalization):
    ...
    if normalization == "layernorm":
        out_batch, norm_cache = layernorm_forward(a, gamma, beta, bn_param)
    elif normalization == "batchnorm":
        out_batch, norm_cache = batchnorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(out_batch)
    cache = (fc_cache, norm_cache, relu_cache)
    return out, cache

def relu_norm_affine_backward(dout, cache, normalization):
    fc_cache, norm_cache, relu_cache = cache
    d_relu = relu_backward(dout, relu_cache)
    if normalization == "batchnorm":
        d_norm, dgamma, dbeta = batchnorm_backward_alt(d_relu, norm_cache)
    elif normalization == "layernorm":
        d_norm, dgamma, dbeta = layernorm_backward(d_relu, norm_cache)
    
    dx, dw, db = affine_backward(d_norm, fc_cache)
