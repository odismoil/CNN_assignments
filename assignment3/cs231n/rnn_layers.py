...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    dx_T, dh0, dWx, dWh, db = rnn_step_backward(dh[:,T-1,:], cache[T-1])
    D = dx_T.shape[1]
    dx = np.zeros((N,T,D))
    dx[:,T-1,:] = dx_T
    for i in range(T-1):
        dh0 += dh[:,T-2-i,:]
        dx[:,T-2-i,:], dh0, d_Wx, d_Wh, d_beta = rnn_step_backward(dh0, cache[T-2-i])
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    a_i = activation[:,:H]; a_f = activation[:,H:2*H];
    a_o = activation[:,2*H:3*H]; a_g = activation[:,3*H:]
    
    i_gate = sigmoid(a_i); f_gate = sigmoid(a_f)
    o_gate = sigmoid(a_o); g_gate = np.tanh(a_g)
    
    next_c = f_gate*prev_c + i_gate*g_gate
    next_h = o_gate*np.tanh(next_c)
    
    cache = (i_gate, f_gate, o_gate, g_gate, a_i, a_f, a_o, a_g, x, prev_h, prev_c, Wx, Wh, b, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    i_gate, f_gate, o_gate, g_gate, a_i, a_f, a_o, a_g, x, prev_h, prev_c, Wx, Wh, b, next_c, next_h = cache
    
    do_gate = dnext_h*np.tanh(next_c); da_o = do_gate*sigmoid(a_o)*(1-sigmoid(a_o))
    dc = dnext_h*o_gate*(1-np.tanh(next_c)**2) + dnext_c
    di_gate = dc*g_gate; da_i = di_gate*sigmoid(a_i)*(1-sigmoid(a_i))
    df_gate = dc*prev_c; da_f = df_gate*sigmoid(a_f)*(1-sigmoid(a_f))
    dg_gate = dc*i_gate; da_g = dg_gate*(1-np.tanh(a_g)**2)
    dprev_c = dc*f_gate
    
    d_activation = np.hstack((da_i, da_f, da_o, da_g))
    dx =d_activation.dot(Wx.T)
    dWx = x.T.dot(d_activation)
    dprev_h = d_activation.dot(Wh.T); dWh = prev_h.T.dot(d_activation)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        next_h, next_c, caches[i] = lstm_step_forward(x[:,i,:], h0, c0, Wx, Wh, b)
        h[:,i,:] = next_h
...
def lstm_backward(dh, cache):
    ...
    for i in range(T,0,-1):
        dh_next = dh[:, i-1, :] + dh0
        dx[:,i-1,:], dh0, dc, d_Wx, d_Wh, d_b = lstm_step_backward(dh_next, dc, caches[i-1])
        dWx += d_Wx
        dWh += d_Wh
        db += d_b
